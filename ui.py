...
def _crossref_headers() -> dict[str, str]:
    ...
    ua = f"paperscout/2.0 (+mailto:{mailto})"
...
def _extract_issued_date(item: dict) -> Optional[str]:
...
def resolve_issn_by_title(journal_title: str) -> Tuple[Optional[str], str]:
    ...
    with httpx.Client(timeout=30.0, headers=_crossref_headers()) as c:
        r = c.get(f"{CR_BASE}/journals", params=params)
        r.raise_for_status()
        items = r.json().get("message", {}).get("items", [])
    jt_norm = _norm_title(journal_title)
    best = None
    for it in items:
        title = it.get("title") or ""
        if _norm_title(title) == jt_norm:
            best = it
            break
...
@retry(wait=wait_exponential(multiplier=1, min=1, max=20), stop=stop_after_attempt(5))
def fetch_from_crossref_strict(
    journal: str,
    rows: int = 50,
    ...
    if since:
        filt.append(f"from-pub-date:{since}")
    if until:
        filt.append(f"until-pub-date:{until}")
    if issn:
        filt.append(f"issn:{issn}")
    else:
    ...
    jt = _norm_title(journal)
    arts: List[Article] = []
    for it in items:
        cont = " ".join(it.get("container-title") or []).strip()
        cont_norm = _norm_title(cont)
        its_issn = [s.lower() for s in (it.get("ISSN") or [])]
        ok = False
        if issn and any(s == issn.lower() for s in its_issn):
            ok = True
        elif cont_norm == jt:
            ok = True
        if not ok:
        ...
        issued = _extract_issued_date(it)
